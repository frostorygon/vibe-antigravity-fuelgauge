/**
 * Antigravity FuelGauge - Platform Strategies
 * 针对不同操作System的Process检测策略
 */
...
/**
 * Windows Platform Strategies
 */
export class WindowsStrategy implements PlatformStrategy {
    /**
     * 判断命令行是否属于 Antigravity Process
     * 精准匹配：必须同时满足以下条件：
     * 1. 必须有 --extension_server_port Parameter
     * 2. 必须有 --csrf_token Parameter
     * 3. 必须有 --app_data_dir antigravity Parameter
     */
    private isAntigravityProcess(commandLine: string): boolean {
        // 条件1：必须包含 --extension_server_port Parameter
        if (!commandLine.includes('--extension_server_port')) {
            return false;
        }

        // 条件2：必须包含 --csrf_token Parameter
        if (!commandLine.includes('--csrf_token')) {
            return false;
        }

        // 条件3：必须有 --app_data_dir antigravity Parameter（最可靠的标识）
        return /--app_data_dir\s+antigravity\b/i.test(commandLine);
    }

    /**
     * 按Process名GetProcessList命令
     * 仅使用 PowerShell
     */
    getProcessListCommand(processName: string): string {
        const utf8Header = '[Console]::OutputEncoding = [System.Text.Encoding]::UTF8; ';
        // 使用单引号包裹 Filter Parameter，内部 name 值使用双单引号转义
        // chcp 65001 >nul 确保 CMD Environment以 UTF-8 Running，避免乱码
    ...
    /**
     * 按关键字GetProcessList命令（查找所有包含 csrf_token 的Process）
     * 这是备用方案，当按Process名查找Failed时使用
     */
    getProcessByKeywordCommand(): string {
        const utf8Header = '[Console]::OutputEncoding = [System.Text.Encoding]::UTF8; ';
        // chcp 65001 >nul 确保 CMD Environment以 UTF-8 Running
...
export class UnixStrategy implements PlatformStrategy {
    ...
    getProcessListCommand(processName: string): string {
        ...
        // -ww: 无限宽度
        // -eo: Custom输出格式
        // pid,ppid,args: ProcessID、父ProcessID、完整命令行
    ...
    parseProcessInfo(stdout: string): ProcessInfo[] {
        ...
        for (const line of lines) {
            ...
            // 必须同时满足：有 csrf_token 且是 Antigravity Process
        ...
        // Unix PlatformSort策略：CurrentProcess的子Process > 其他Process
        // 为了提高Success率，我们将子Process排在第一位，但Return所有候选Process
    ...
    getDiagnosticCommand(): string {
        // 列出所有包含 'language' 或 'antigravity' 的Process
